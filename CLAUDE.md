...
- **PWA & Offline-First:** **Serwist (`@serwist/next`) + Turbopack** for service worker lifecycle, route/data caching (*stale-while-revalidate* on asset pages, *cache-first* on calculators), and offline installability.
- **TanStack Ecosystem:**
...
- **Frontend-Backend Communication:** Standard **REST + OpenAPI (Scalar / Swagger)** with typed client generation in Next.js (`@hey-api/openapi-ts` or typed fetch). *(oRPC / Elysia+Bun are dropped for now, reserved for future consideration if needed).*
...
Shared infra via `docker-compose.yml`: **PostgreSQL 18 + TimescaleDB (self-hosted)** *(fallback to PostgreSQL 18 partitioned by date if needed)*, **Redis**, **RabbitMQ**, **Jaeger**.
