...
const VALUE_NUDGE_KEYS = new Set(['ArrowUp', 'ArrowDown', 'PageUp', 'PageDown']);
...
export function ScrubLogoIcon({ className, name }: { className?: string; name: LogoIconName }) {
...
export function normalizeScrubFieldSettings(settings: ScrubFieldSettings): ScrubFieldSettings {
  ...
  const logoIcon =
    requestedLogoIcon && LOGO_ICON_OPTIONS.includes(requestedLogoIcon as LogoIconName)
      ? requestedLogoIcon
...
function placeCaretAtPoint(input: HTMLInputElement, clientX: number, clientY: number) {
...
function ScrubBoundFeedback({
  ...
  useEffect(() => {
    ...
    const shakeTargets = wrapRef.current
      ? Array.from(wrapRef.current.querySelectorAll<HTMLElement>('.scrub-bound-field'))
      : [];
...
export function useNumberScrub({
  ...
  const [boundFeedback, setBoundFeedback] = useState<BoundFeedbackState | null>(null);
  ...
  const triggerBoundFeedback = useCallback(
    (edge: 'min' | 'max', source: BoundFeedbackSource, attempted: number) => {
  ...
  const commit = useCallback(
    ...
    [formatForEdit, getCurrentNumericValue, max, min, onChange, step, triggerBoundFeedback],
  );
  ...
  }, [applyWheelNudge, disabled, logoScrollEnabled, resetWheelAccumulator, scrub.wheelEnabled]);
  ...
  const attachScrubSessionGuard = useCallback(() => {
    ...
    const handleGlobalPointerEnd = (event: globalThis.PointerEvent) => {
      ...
      endScrubSession(event as unknown as PointerEvent<HTMLElement>, state.source === 'input');
    };
  ...
  const activateScrubbing = useCallback(
    (event: PointerEvent<HTMLElement>, captureTarget: HTMLElement | null) => {
      const state = scrubRef.current;
  ...
  const applyScrubDelta = useCallback(
    (event: PointerEvent<HTMLElement>) => {
      ...
      const scrubDirection = effectiveDelta === 0 ? undefined : effectiveDelta > 0 ? 1 : -1;
      commit(
        attempted,
        'scrub',
  ...
  const inputProps = {
    ...
    onBlur: () => {
      ...
      if (Number.isFinite(parsed)) {
        ...
        const bounded = commit(parsed, undefined, resolveCommitQuantizeStep(parsed));
        notifyCommit(bounded);
        return;
      }
...
function CalligraphNumber({
  ...
  const animation = settings.animation === 'default' ? undefined : settings.animation;

  if (shouldReduceMotion) {
    return (
      <span ref={contentRef} data-slot="scrub-number-calligraph-content" style={style}>
        {value}
      </span>
    );
  }
  ...
  const scrubSurface = scrub.editing ? (
    ...
    <div
      ...
      className={cn(
        ...
        usesGroupedControl
          ? groupControlClass
          : cn('relative z-[1] flex items-center justify-start text-foreground scrub-bound-field'),
        !logoScrollEnabled &&
          scrub.canScrub &&
  ...
  const fieldContent = (
    ...
    >
      <Input
        ref={mirrorRef}
        aria-hidden
        aria-label={typeof ariaLabel === 'string' ? ariaLabel : 'Scrub number value'}
      ...
      {logoScrollEnabled ? (
        <InputGroupAddon
          ...
          className={cn(
            ...
            scrub.canScrub && getScrubCursorClass(scrubSettings, scrub.atBound, scrubBounds),
          )}
...
export function ScrubNumberField({
  ...
  const formatValue =
    formatValueProp ??
    (format
      ? (nextValue: number) => new Intl.NumberFormat(undefined, format).format(nextValue)
      : undefined);
  ...
  const field = (
    <div className="min-w-0">
      <ScrubNumberInput
        ...
        className={cn(
          grouped ? 'min-w-0 flex-1' : logo.enabled ? 'w-[6.75rem]' : 'w-[4.75rem]',
          className,
        )}
        disabled={disabled}
        grouped={grouped}
  ...
  return (
    <div className="flex items-center gap-3">
      <span className={cn('w-16 text-sm font-medium text-muted-foreground', labelClassName)}>
        {label}
      </span>
      {field}
    </div>
