...
export function pickAlternateModifier(current: ModifierKey, exclude: ModifierKey): ModifierKey {
...
export function resolveFineModifierKey(modifier: FineModifier = 'alt'): ModifierKey {
  return modifier;
}

export function resolveCoarseModifierKey(modifier: CoarseModifier = 'shift'): ModifierKey {
  return modifier;
}

export function isFineModifierPressed(event: ModifierKeys, modifier: FineModifier = 'alt') {
  return isModifierKeyPressed(event, resolveFineModifierKey(modifier));
}

export function isCoarseModifierPressed(event: ModifierKeys, modifier: CoarseModifier = 'shift') {
...
export function toModifierKeys(event: {
  ...
  return {
    ...
    getModifierState: event.getModifierState ? (key) => event.getModifierState!(key) : undefined,
  };
}
...
export function normalizeFiniteNumber(value: number | undefined): number | undefined {
...
export function normalizePositiveFiniteStep(step: number | undefined, fallback = 1) {
...
export function normalizeWheelSensitivity(sensitivity: number | undefined, fallback = 20) {
  return clampNumber(normalizePositiveFiniteStep(sensitivity, fallback), 1, 200);
}

export function normalizeScrubThreshold(threshold: number | undefined, fallback = 3) {
  const candidate =
    typeof threshold === 'number' && Number.isFinite(threshold) ? threshold : fallback;
...
export function getAtBound(value: number, min?: number, max?: number): 'min' | 'max' | null {
...
export function boundOverflow(attempted: number, edge: 'min' | 'max', min?: number, max?: number) {
...
export function preserveDisplayDraft(currentDraft: string, value: number, fallback: string) {
