...
export function computeCatalogStats(assets: AssetDto[] = []): CatalogStatsSummary {
  const prices = assets
    .map((asset) => numeric(asset, 'lastPrice'))
    .filter((value): value is number => value != null);
  const returns = assets
    .map((asset) => numeric(asset, 'return12mPercent'))
    .filter((value): value is number => value != null);
  ...
  return {
    ...
    primaryMetric: {
      ...
      value: returns.length
        ? `${(returns.reduce((sum, value) => sum + value, 0) / returns.length).toFixed(2)}%`
        : '—',
    },
    secondaryMetric: {
      label: 'Cotação média',
      value: prices.length
        ? `R$ ${(prices.reduce((sum, value) => sum + value, 0) / prices.length).toFixed(2)}`
        : '—',
    },
