...
function checkSourceRefs(errors: string[], refs: any[], location: string): void {
  ...
  refs.forEach((ref, index) => {
    if (!isNonEmptyString(ref?.file) || !isNonEmptyString(ref?.section)) {
      ...
      addError(errors, `${location}.source_refs[${index}]: source file does not exist: ${ref.file}`);
    }
  });
}
...
function checkTask(errors: string[], task: any, phaseId: string, epicId: string): void {
  ...
  const required = ["id", "title", "description", "status", "priority", "difficulty", "kind", "phase_id", "epic_id", "depends_on", "blocks", "source_refs", "acceptance_criteria", "deliverables", "risks", "notes"];
  ...
  if (!isNonEmptyString(task?.description)) addError(errors, `${location}: description is required`);
  if (!statuses.has(task?.status)) addError(errors, `${location}: invalid status ${task?.status}`);
  if (!priorities.has(task?.priority)) addError(errors, `${location}: invalid priority ${task?.priority}`);
  if (!difficulties.has(task?.difficulty)) addError(errors, `${location}: invalid difficulty ${task?.difficulty}`);
  ...
  if (!Array.isArray(task?.depends_on)) addError(errors, `${location}: depends_on must be an array`);
  if (!Array.isArray(task?.blocks)) addError(errors, `${location}: blocks must be an array`);
  if (!Array.isArray(task?.acceptance_criteria) || task.acceptance_criteria.length === 0) addError(errors, `${location}: acceptance_criteria must not be empty`);
  if (!Array.isArray(task?.deliverables) || task.deliverables.length === 0) addError(errors, `${location}: deliverables must not be empty`);
...
export function validateRoadmap(data: RoadmapData): string[] {
  ...
  if (root?.document_type !== "roadmap") addError(errors, "roadmap.json: document_type must be roadmap");
  if (root?.schema_version !== "1.0.0") addError(errors, "roadmap.json: unsupported schema_version");
  if (root?.current_state?.implementation_status !== "documentation_only") {
    addError(errors, "roadmap.json: implementation_status must be documentation_only while product code is absent");
  }
  if (root?.current_state?.roadmap_tracking_scaffolded !== true) addError(errors, "roadmap.json: roadmap_tracking_scaffolded must be true");
  if (root?.current_state?.code_scaffolded !== false) addError(errors, "roadmap.json: code_scaffolded must be false while product code is absent");
  if (!Array.isArray(root?.phase_order) || root.phase_order.length !== phases.length) addError(errors, "roadmap.json: phase_order must list every phase file");
  ...
  for (const phaseDocument of phases) {
    ...
    if (phaseDocument?.document_type !== "phase") addError(errors, `${phaseLocation}: document_type must be phase`);
    register(phase?.id, phaseLocation);
    if (!statuses.has(phase?.status)) addError(errors, `${phaseLocation}: invalid status ${phase?.status}`);
    if (!priorities.has(phase?.priority)) addError(errors, `${phaseLocation}: invalid priority ${phase?.priority}`);
    if (!Array.isArray(phase?.depends_on)) addError(errors, `${phaseLocation}: depends_on must be an array`);
    ...
    for (const epic of phase.epics) {
      ...
      if (!statuses.has(epic?.status)) addError(errors, `${epicLocation}: invalid status ${epic?.status}`);
      if (!priorities.has(epic?.priority)) addError(errors, `${epicLocation}: invalid priority ${epic?.priority}`);
      if (!Array.isArray(epic?.depends_on)) addError(errors, `${epicLocation}: depends_on must be an array`);
  ...
  for (const decision of decisions?.decisions ?? []) {
    ...
    if (!["open", "accepted", "rejected", "superseded"].includes(decision?.status)) addError(errors, `${location}: invalid status`);
  ...
  for (const risk of risks?.risks ?? []) {
    ...
    if (!["open", "mitigated", "accepted", "closed"].includes(risk?.status)) addError(errors, `${location}: invalid status`);
    if (!["low", "medium", "high"].includes(risk?.probability)) addError(errors, `${location}: invalid probability`);
    if (!["low", "medium", "high", "critical"].includes(risk?.impact)) addError(errors, `${location}: invalid impact`);
  ...
  const completeTasks = [...ids.keys()].filter((id) => id.startsWith("FND-") || id.startsWith("MVP-") || id.startsWith("GROW-") || id.startsWith("PORT-"));
  const recorded = new Set(root?.current_state?.completed_task_ids ?? []);
  for (const id of recorded) {
    if (!completeTasks.includes(id)) addError(errors, `current_state.completed_task_ids contains unknown task ${id}`);
  }
...
if (import.meta.main) {
  try {
    ...
    const taskCount = data.phases.reduce((count, item) => count + item.phase.epics.reduce((epicCount: number, epic: any) => epicCount + epic.tasks.length, 0), 0);
    console.log(`Roadmap valid: ${data.phases.length} phases, ${taskCount} tasks, ${data.decisions.decisions.length} decisions, ${data.risks.risks.length} risks.`);
  } catch (error) {
