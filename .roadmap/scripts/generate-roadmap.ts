...
function renderPhase(phase: any): string[] {
  ...
  for (const epic of phase.epics) {
    ...
    lines.push(
      ...
      '',
      `**Status:** ${statusIcon(epic.status)} \`${epic.status}\` · **Prioridade:** \`${epic.priority}\` · **Progresso:** ${bar(epicPercentage, 12)} ${epicPercentage}% (${done}/${epic.tasks.length})`,
      '',
    );
    lines.push(
    ...
    for (const task of epic.tasks) {
      const dependencies =
        task.depends_on.length > 0
          ? task.depends_on.map((id: string) => `\`${id}\``).join(', ')
          : '—';
      lines.push(
        `| \`${task.id}\` | ${escapeTable(task.title)} | \`${task.priority}\` | \`${task.difficulty}\` | ${statusIcon(task.status)} \`${task.status}\` | ${dependencies} |`,
      );
    }
...
function render(data: RoadmapData): string {
  ...
  for (const document of data.phases) {
    ...
    const stats = phaseStats(phase);
    lines.push(
      `| **${String(phase.order).padStart(2, '0')} — ${phase.name}** | ${statusIcon(phase.status)} \`${phase.status}\` | \`${phase.priority}\` | ${stats.percentage}% (${stats.done}/${stats.tasks.length}) | ${phase.depends_on.length ? phase.depends_on.map((id: string) => `\`${id}\``).join(', ') : '—'} |`,
    );
  }
  ...
  for (const decision of data.decisions.decisions)
    lines.push(
      `| \`${decision.id}\` | ${escapeTable(decision.title)} | \`${decision.status}\` | \`${decision.priority}\` | ${escapeTable(decision.recommendation)} |`,
    );
  ...
  for (const risk of data.risks.risks)
    lines.push(
      `| \`${risk.id}\` | ${escapeTable(risk.title)} | \`${risk.probability}\` | \`${risk.impact}\` | \`${risk.status}\` | ${escapeTable(risk.mitigation)} |`,
    );
  ...
  lines.push(
    ...
    '## Documentos-fonte',
    '',
    ...data.root.source_documents.map((source: any) => `- \`${source.file}\` — ${source.section}`),
    '',
    '## Como atualizar',
    '',
